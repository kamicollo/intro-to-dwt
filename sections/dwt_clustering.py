...
def build_scalogram(signals, sel_signals, decomp_func):
    ...
    sc_df = pd.DataFrame(scalogram).melt(
        id_vars=["key", "type"], var_name="Resolution", value_name="Energy"
    )

    names = ["S"] + ["D" + str(i) for i in range(1, 12)]
    c = (
        alt.Chart(sc_df, height=400)
        .mark_line()
        .encode(
            ...
            color=alt.Color(
                ...
                scale=alt.Scale(
                    domain=[
                        "Unimpaired signal",
                        "Wave impairment + upstream modulation",
                        "Tilt / wave impairment",
                        "Wave impairment",
                    ],
